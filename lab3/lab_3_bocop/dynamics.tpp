...

{
	...
	Tdouble i2 = state[1];
	Tdouble u = control[0];
	double alph = constants[0];
	double L1 = constants[1];
	double L2 = constants[2];
	double Rc = constants[3];
	double Rw = constants[4];
	double K =alph * pow(L1*L2, 0.5);

	state_dynamics[0] = 1/((1-pow(alph, 2)) * L1 * L2) * (-i1 * L2*Rc	+ i2 * alph*pow(L1*L2, 0.5)*Rw + L2*u);
	state_dynamics[1] = 1/((1-pow(alph, 2)) * L1 * L2) * (alph*pow(L1*L2, 0.5)*Rc*i1 - L1*Rw*i2 -K*u);
}
